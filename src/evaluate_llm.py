...
class EvaluateLLM:
    ...
    def evaluate(self, dataset_dict):
        ...
        eval_results = trainer.evaluate(eval_dataset=tokenized_data["test"])
        return eval_results
...
if __name__ == '__main__':
    ...
    model_path = "/home/usd.local/anushuya.baidya/anushuya/bias_mitigation_large_language_models/results/xlnet_fine-tuned/"

    evaluate_llm = EvaluateLLM(model_path=model_path, device=device)
    with open(dataset_path, 'r', encoding='utf-8') as f:
        data = json.load(f)

    dataset_dict = evaluate_llm.create_dataset(data)
    results = evaluate_llm.evaluate(dataset_dict)
